...
void displayCardInfo(NxpNci_RfIntf_t RfIntf)
{
    ...
    case (MODE_POLL | TECH_PASSIVE_NFCA):
        ...
        //char stringTagID[14]={0};
             char twobytes[2]={0};

             for(int i=0;i<RfIntf.Info.NFC_APP.NfcIdLen;i++)
             {

             	sprintf(twobytes,"%.2x",RfIntf.Info.NFC_APP.NfcId[i]);
                strcat(TagUIDstr,twobytes);
             }
             //strcpy(TagUIDstr,stringTagID);
